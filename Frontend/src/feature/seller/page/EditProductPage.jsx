...
function SectionHeader({ title, sub }) {
  return (
    <div className="border-b pb-4" style={{ borderColor: "#242424" }}>
      ...
      {sub && (
        <p className="text-xs mt-0.5" style={{ color: "#9a9078" }}>
          {sub}
        </p>
      )}
    </div>
    ...
    >
      {selected && (
        <svg
          ...
          viewBox="0 0 24 24"
          fill="none"
          stroke={color.hex === "#f0ede8" ? "#111" : "#fff"}
          strokeWidth="3"
          strokeLinecap="round"
        ...
        <div className="flex flex-col items-center gap-3 p-8">
          ...
          <div className="text-center">
            ...
            <p className="text-xs mt-1" style={{ color: "#9a9078" }}>
              Drag & drop or{" "}
              <span style={{ color: "#f5c518" }}>click to browse</span>
            </p>
...
export default function EditProductPage() {
      ...
      try {
        ...
        if (data && data.product) {
          ...
          const inferredGender =
            p.gender ||
            p.tags?.find((t) => ["Men", "Women", "Unisex"].includes(t)) ||
            "Men";
          // Infer subcategory from tags or property
          const inferredSub =
            p.subcategory ||
            p.tags?.find((t) => t.startsWith("sub:"))?.replace("sub:", "") ||
            "";
  ...
  const toggleArrayItem = (key, value) => {
    setForm((prev) => {
      ...
      if (key === "sizes" || key === "colors") {
        ...
        updatedForm.variants = generateVariantCombinations(
          nextSizes,
          nextColors,
          prev.variants,
        );
  ...
  const handleGalleryFiles = (e) => {
    ...
    const combined = [...galleryFiles, ...selected].slice(
      0,
      Math.max(0, totalAllowed),
    );
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      form.compareAtPrice &&
      (isNaN(form.compareAtPrice) ||
        Number(form.compareAtPrice) < Number(form.price))
    )
      return setError(
        "Compare at price (MRP / Original Price) must be greater than or equal to the selling price.",
      );
    if (!coverFile && !coverPreview)
      return setError("A cover image is required.");
      ...
      if (form.colorPrices) {
        Object.entries(form.colorPrices).forEach(([c, val]) => {
          ...
          if (typeof val === "object") {
            ...
            const cap = val.compare_at_price
              ? parseFloat(val.compare_at_price)
              : null;
            if (!isNaN(p) && p >= 0) {
              cleanColorPrices[c] = {
                price: p,
                ...(cap && cap >= p ? { compare_at_price: cap } : {}),
              };
            }
  ...
  if (isFetching) {
    return (
      ...
      >
        <p className="text-sm font-bold text-[#f5c518] animate-pulse">
    ...
    >
      ...
      <main className="pt-[68px] max-w-[1200px] mx-auto px-8 py-12">
        ...
        {error && (
          <div
            role="alert"
            className="rounded p-4 mb-8 text-sm font-medium bg-red-950/40 border border-red-800/60 text-red-300"
          >
            {error}
          </div>
        )}
        {success && (
          <div
            role="status"
            className="rounded p-4 mb-8 text-sm font-medium bg-emerald-950/40 border border-emerald-800/60 text-emerald-300"
          >
            {success}
          </div>
        )}
          ...
          <div className="flex-1 flex flex-col gap-8 w-full">
            ...
            <FormCard>
              <SectionHeader
                ...
                sub="Department, Category, and Specs"
              />
              ...
              <div className="flex flex-col gap-6">
                    ...
                    {Object.entries(CATEGORY_TREE).map(([catKey, catObj]) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] text-[#f5c518]/70 font-mono tracking-widest">
                            {catObj.tag}
                          </span>
                        </button>
                      );
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  <label
                    ...
                    className="text-xs font-semibold uppercase tracking-wider text-[#e5e2e1] cursor-pointer"
                  >
            ...
            <FormCard>
              <SectionHeader
                ...
                sub="Set your selling price and available units"
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold pointer-events-none text-[#f5c518]">
                      ₹
                    </span>
                    <input
                      id="product-price"
                ...
                {/* Compare at price */}
                <div>
                  <FieldLabel htmlFor="product-compare">
                    Compare at (MRP ₹)
                  </FieldLabel>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold pointer-events-none text-[#9a9078]">
                      ₹
                    </span>
                    <input
                      id="product-compare"
              ...
              {form.compareAtPrice &&
                ...
                Number(form.compareAtPrice) > Number(form.price) && (
                  <p className="mt-2 text-xs text-[#9a9078]">
                    ...
                    — discount badge will display on storefront.
                  </p>
                )}
              ...
              {form.compareAtPrice &&
                ...
                Number(form.compareAtPrice) < Number(form.price) && (
                  <p className="mt-2 text-xs text-red-400">
                    Compare at price (MRP) must be greater than or equal to
                    selling price.
                  </p>
                )}
            </FormCard>
                    ...
                    <div className="mt-5 pt-4 border-t border-[#262626]">
                      <div className="mb-3">
                        ...
                        <p className="text-[11px] text-[#888] mt-1">
                          Set custom prices for special colors or limited
                          editions. Colors left blank will automatically sell at
                          the Base Price (₹{form.price || "0"}).
                        </p>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                        {form.colors.map((c) => {
                          const colorData = COLOR_OPTIONS.find(
                            (opt) => opt.label === c,
                          ) || { hex: "#888" };
                          ...
                          return (
                            <div
                              key={c}
                              className="p-3 rounded-lg bg-[#141414] border border-[#2a2a2a] flex flex-col gap-2"
                            >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold uppercase tracking-wider text-[#f5c518]">
                            ...
                            {form.variants?.length || 0} Combinations)
                          </span>
                        ...
                        <p className="text-[11px] text-[#888] mt-1">
                          Combine <strong>Sizes & Colors</strong>. Any field you
                          leave blank automatically uses the product's default
                          (Price: ₹{form.price || "0"}, Stock:{" "}
                          {form.stock || "0"}).
                        </p>
                      </div>
                      <button
                            ...
                            >
                              ...
                              <td className="p-2">
                                <input
                                  ...
                                  placeholder={
                                    form.sku
                                      ? `${form.sku}-${v.attributes?.size}-${v.attributes?.color}`
                                      : "SKU"
                                  }
                                  value={v.sku ?? ""}
                                  onChange={(e) =>
            ...
            <FormCard>
              <SectionHeader
                ...
                sub="Upload high-res photos to showcase the garment"
              />

              {/* Cover Image */}
              <div className="mb-5">
                <FieldLabel htmlFor="cover-upload" required>
                  Cover Image (Primary thumbnail)
                </FieldLabel>
                <ImageUploadZone
                  id="cover-upload"
              ...
              <div>
                ...
                >
                  ...
                  {galleryPreviews.map((src, i) => (
                    <div
                      key={`new-${i}`}
                      className="relative rounded group overflow-hidden border border-[#f5c518]/60 bg-[#161616]"
                      style={{ aspectRatio: "1" }}
                    >
                      <img
                        src={src}
                ...
                <p className="mt-2.5 text-xs text-[#777]">
                  Click &ldquo;+ Add Photos&rdquo; to add more images to your
                  product gallery.
                </p>
              </div>
            </FormCard>
